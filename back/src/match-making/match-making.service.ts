...
import {MATCH_TYPES, MatchType} from './dto/AppendToQueueDTO';
...
export class MatchMakingService {
  ...
  // PERF: we could save the queue the user is on in the database for better
  // dequeueing
  enqueue(user: Express.User, matchType: MatchType) {
    ...
    this.logger.log(
        `user ${user.login_intra} is entering the ${
            matchType} match-making queue`,
    );
...
private createMatch(queue: Express.User[]): MemoryMatch {
  ...
  this.logger.debug(
      `Creating a match between users ${user1.login_intra} and ${
          user2.login_intra}`,
  );
